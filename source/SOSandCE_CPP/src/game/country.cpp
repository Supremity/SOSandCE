...
void Country::update(GameState& gs) {
            ...
            for (auto& jb : justBuilt) {
                ...
                if (gs.industryMapSurf) {
                    ...
                    if (gs.regionsMapSurf) {
                        MapFunc::fillRegionMask(gs.industryMapSurf, gs.regionsMapSurf, loc.x, loc.y, buildingMapColor(jb.type));
                    } else {
    ...
    if (gs.controlledCountry != name && gs.speed > 0) {
        runAI(gs);
    }
...
void Country::addRegion(int id, GameState& gs, bool ignoreFill, int divRegion, bool ignoreResources) {
    ...
    if (!ignoreFill) {
        ...
        if (gs.politicalMapSurf) {
            if (gs.regionsMapSurf) {
                MapFunc::fillRegionMask(gs.politicalMapSurf, gs.regionsMapSurf, px, py, color);
            } else {
        ...
        if (gs.ideologyMapSurf) {
            ...
            if (gs.regionsMapSurf) {
                MapFunc::fillRegionMask(gs.ideologyMapSurf, gs.regionsMapSurf, px, py, ideColor);
            } else {
        ...
        if (gs.factionMapSurf) {
            if (gs.regionsMapSurf) {
                MapFunc::fillRegionMask(gs.factionMapSurf, gs.regionsMapSurf, px, py, factionColor);
            } else {
...
void Country::runAI(GameState& gs) {
    ...
    for (int i = static_cast<int>(training.size()) - 1; i >= 0; --i) {
        if (training[i][1] == 0) {
            ...
            addDivision(gs, training[i][0], deployReg, true);
...
void Country::addFactory(GameState& gs, int region) {
    ...
    if (region >= 0 && std::find(regions.begin(), regions.end(), region) != regions.end()) {
        ...
        if (gs.industryMapSurf) {
            ...
            if (gs.regionsMapSurf) {
                MapFunc::fillRegionMask(gs.industryMapSurf, gs.regionsMapSurf, loc.x, loc.y, buildingMapColor(BuildingType::CivilianFactory));
            } else {
...
void Country::addPort(GameState& gs, int region) {
    ...
    if (gs.industryMapSurf) {
        Vec2 loc = rd.getLocation(region);
        if (gs.regionsMapSurf) {
            MapFunc::fillRegionMask(gs.industryMapSurf, gs.regionsMapSurf, loc.x, loc.y, buildingMapColor(BuildingType::Port));
        } else {
...
void Country::build(GameState& gs, const std::string& n, int days, int region) {
    ...
    if (gs.industryMapSurf) {
        ...
        if (gs.regionsMapSurf) {
            MapFunc::fillRegionMask(gs.industryMapSurf, gs.regionsMapSurf, loc.x, loc.y, constructColor);
        } else {
...
void Country::destroy(GameState& gs, int region) {
    ...
    if (!blist.empty()) {
        ...
        if (gs.industryMapSurf) {
            ...
            if (gs.regionsMapSurf) {
                MapFunc::fillRegionMask(gs.industryMapSurf, gs.regionsMapSurf, loc.x, loc.y, paintColor);
            } else {
