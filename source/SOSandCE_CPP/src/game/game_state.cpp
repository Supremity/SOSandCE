...
void GameState::peaceTreaty(const std::string& mainCountry) {
    ...
    for (auto& victor : victors) {
        if (victor == controlledCountry) continue;
        auto* v = getCountry(victor);
        if (!v) continue;
        for (auto& enemy : combatants) {
            auto* e = getCountry(enemy);
            if (!e) continue;
            if (v->culture == e->culture) {
                v->annexCountry(v->culture, *this, enemy);
            } else if (e->cultures.count(v->culture) && !e->cultures[v->culture].empty()) {
                v->addRegions(e->cultures[v->culture], *this);
            }
        }
    }
    ...
    if (std::find(victors.begin(), victors.end(), controlledCountry) != victors.end()) {
