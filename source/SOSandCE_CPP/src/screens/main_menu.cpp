...
void MainMenuScreen::handleInput(App& app, const InputState& input) {
        ...
        if (clicked) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (mx >= bX && mx <= bX + bW && my >= by && my <= by + bH) {
                    ...
                    if (i == 2) { nextScreen = ScreenType::SETTINGS; }
    ...
    case MenuState::COUNTRIES: {
        ...
        if (clicked) {
            ...
            float btnW3 = std::round(winW2 * 0.18f);
            float btnGap3 = std::round(u2 * 1.2f);
            float totalBtnW = btnW3 * 3 + btnGap3 * 2;
...
void MainMenuScreen::render(App& app) {
    ...
    case MenuState::SETTINGS:  nextScreen = ScreenType::SETTINGS; break;
    }
}
...
void MainMenuScreen::renderMainMenu(App& app) {
    ...
    SDL_Texture* btnDefaultTex = assets.btnRectDefault();
    SDL_Texture* btnHoverTex = assets.btnRectHover();
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!isPress) {
            ...
            SDL_Rect bsh = {(int)btnX + 2, (int)by + 3, (int)btnW, (int)btnH};
            SDL_RenderFillRect(r, &bsh);
        }


        SDL_Texture* btnTex = hov ? btnHoverTex : btnDefaultTex;
        if (btnTex) {
            if (isPress)       SDL_SetTextureColorMod(btnTex, 120, 118, 115);
            else if (hov)      SDL_SetTextureColorMod(btnTex, 210, 205, 195);
            else               SDL_SetTextureColorMod(btnTex, 155, 153, 150);
            UIAssets::draw9Slice(r, btnTex, btnX, drawY, btnW, btnH, 16);
...
void MainMenuScreen::renderCountriesMenu(App& app) {
    ...
    float btnW2 = std::round(winW * 0.18f);
    float btnGap2 = std::round(u * 1.2f);
    float totalBtnW = btnW2 * 3 + btnGap2 * 2;
