...
void GameScreen::handleInput(App& app, const InputState& input) {
    ...
    if (input.mouseLeftDown && sidebar_ && sidebar_->isOpen()) {
    ...
    if (sidebar_ && sidebar_->isOpen()) {
        float sideX = sidebar_->getX(W);
        overSidebar = input.mouseX >= sideX;
    }
    ...
    if (input.mouseLeftDown && !overUI && !hasPopups) {
        ...
        if (player) {
            for (auto& div : player->divisions) {
                if (!div) continue;
                float sx = camera_.worldToScreenX(div->location.x, W);
                float sy = camera_.worldToScreenY(div->location.y, H);

                float dw = (float)div->spriteW;
                float dh = (float)div->spriteH;
                if (input.mouseX >= sx - dw/2 && input.mouseX <= sx + dw/2 &&
                    input.mouseY >= sy - dh/2 && input.mouseY <= sy + dh/2) {
...
void GameScreen::handleMapClick(App& app, const InputState& input) {
    ...
    if (!owner.empty()) {
        ...
        if (sidebar_) {
            ...
            sidebar_->selectedCountry = owner;
            sidebar_->open();
    ...
    if (!selectedDivisions_.empty()) {
        for (auto* div : selectedDivisions_) {
            if (div) {
                div->command(regionId, gs, false, false, 300);
            }
        }
        Audio::instance().playSound("moveDivSound");
        selectedDivisions_.clear();
            ...
            if (owner != gs.controlledCountry) {
                ...
                if (buildMode == "destroy") {
                    ...
                    if (!player->buildingManager.canBuild(regionId, bt)) {
                        ...
                        player->build(gs, buildMode, days, regionId);
...
void GameScreen::handleKeyboard(App& app, const InputState& input) {
        ...
        if (!uiManager_->popupList.empty()) {
            ...
            auto popup = std::make_unique<Popup>(
                ...
                std::vector<PopupButton>{
                    ...
                    PopupButton{"Settings",     "", [appPtr]() { appPtr->switchScreen(ScreenType::SETTINGS); },     0, y0 + s * 2, 4.5f},
    ...
    if (input.isKeyDown(SDL_SCANCODE_1)) { mapManager_.setMode(MapMode::POLITICAL); toasts().show("Political Map"); }
    if (input.isKeyDown(SDL_SCANCODE_2)) { mapManager_.setMode(MapMode::FACTION); toasts().show("Faction Map"); }
    if (input.isKeyDown(SDL_SCANCODE_3)) { mapManager_.setMode(MapMode::IDEOLOGY); toasts().show("Ideology Map"); }
    if (input.isKeyDown(SDL_SCANCODE_4)) { mapManager_.setMode(MapMode::INDUSTRY); toasts().show("Industry Map"); }
    if (input.isKeyDown(SDL_SCANCODE_5)) { mapManager_.setMode(MapMode::BIOME); toasts().show("Biome Map"); }
    if (input.isKeyDown(SDL_SCANCODE_6)) { mapManager_.setMode(MapMode::RESOURCE); toasts().show("Resource Map"); }


    if (input.isKeyDown(SDL_SCANCODE_KP_PLUS) || input.isKeyDown(SDL_SCANCODE_EQUALS)) {
        gs.speed = std::min(gs.speed + 1, 10);
        Audio::instance().playSound("clickedSound");
    }
    if (input.isKeyDown(SDL_SCANCODE_KP_MINUS) || input.isKeyDown(SDL_SCANCODE_MINUS)) {
...
void GameScreen::render(App& app) {
    ...
    {
        std::unordered_map<int, int> dpr, rdc;
        Country* pl = gs.getCountry(gs.controlledCountry);
        if (pl) {
            for (auto& d : pl->divisions) if (d) dpr[d->region]++;
            for (auto& d : pl->divisions) {
                if (!d) continue;
                bool isSel = std::find(selectedDivisions_.begin(), selectedDivisions_.end(), d.get()) != selectedDivisions_.end();
                int myIdx = rdc[d->region]++;
                if (!isSel) continue;
                int total = dpr[d->region];
                float stackOff = d->spriteH * myIdx - d->spriteH * total / 2.0f;
                float sx = camera_.worldToScreenX(d->location.x, W);
                float sy = camera_.worldToScreenY(d->location.y, H) + stackOff;
                int dw = d->spriteW > 0 ? d->spriteW : 30;
                int dh = d->spriteH > 0 ? d->spriteH : 20;
                SDL_SetRenderDrawBlendMode(eng.renderer, SDL_BLENDMODE_BLEND);
                SDL_SetRenderDrawColor(eng.renderer, 255, 255, 80, 180);
                SDL_Rect hl = {(int)(sx - dw/2 - 1), (int)(sy - 1), dw + 2, dh + 2};
                SDL_RenderDrawRect(eng.renderer, &hl);
            }
        }
    }
    ...
    if (!selectedRegions_.empty()) {
        ...
        int lineThick = std::max(1, (int)(H / 100.0f * 0.4f));

        for (int i = 0; i < (int)selectedRegions_.size(); i++) {
            Vec2 loc = rd.getLocation(selectedRegions_[i]);
            float rx = camera_.worldToScreenX(loc.x, W);
            float ry = camera_.worldToScreenY(loc.y, H);

            if (i > 0) {
                SDL_SetRenderDrawColor(eng.renderer, 255, 0, 0, 200);
                for (int t = -lineThick/2; t <= lineThick/2; t++) {
                    SDL_RenderDrawLine(eng.renderer, (int)prevX, (int)prevY + t, (int)rx, (int)ry + t);
                }
            }
            prevX = rx;
            prevY = ry;
...
void GameScreen::renderMap(App& app) {
    ...
    {
        mapRenderer_.renderDivisions(eng.renderer, gs, camera_, W, H);
    }
    ...
    if (!selectedRegions_.empty()) {
        mapRenderer_.renderCommands(eng.renderer, gs, camera_, W, H,
                                     selectedRegions_, xPressed_, yPressed_);
    }
}
...
void GameScreen::renderUI(App& app) {
    ...
    if (player) {
        ...
        for (int ri = 0; ri < RESOURCE_COUNT && ri < 6; ri++) {
            ...
            float netR = rm.production[ri] - rm.consumption[ri];
            Color rc = (netR < 0) ? Theme::red_light : Theme::cream;
            char rbuf[32]; snprintf(rbuf, sizeof(rbuf), "%.0f", stk);
            slots.push_back({resLabels[ri], rbuf, rc, nullptr});
        }
...
void GameScreen::renderPopups(App& app) {
    auto& eng = Engine::instance();
    float uiSize = app.settings().uiSize;
...
void GameScreen::setupGame(App& app, const std::string& scenario, int eventFrequency) {
    ...
    std::string datePath = eng.assetsPath + "starts/" + scenarioName + "/startDate.txt";
    if (fs::exists(datePath)) {
        std::ifstream dateFile(datePath);
        std::string dateStr;
        std::getline(dateFile, dateStr);

        dateStr.erase(std::remove(dateStr.begin(), dateStr.end(), '['), dateStr.end());
        dateStr.erase(std::remove(dateStr.begin(), dateStr.end(), ']'), dateStr.end());
        std::istringstream diss(dateStr);
        std::string tok;
        std::vector<int> dateVals;
        while (std::getline(diss, tok, ',')) {
            tok.erase(0, tok.find_first_not_of(" "));
            tok.erase(tok.find_last_not_of(" ") + 1);
            if (!tok.empty()) dateVals.push_back(std::stoi(tok));
        }
        if (dateVals.size() >= 4) {
            gs.time.hour = dateVals[0];
            gs.time.day = dateVals[1];
            gs.time.month = dateVals[2];
            gs.time.year = dateVals[3];
            gs.time.startHour = dateVals[0];
            gs.time.startDay = dateVals[1];
            gs.time.startMonth = dateVals[2];
            gs.time.startYear = dateVals[3];
...
void GameScreen::showTutorialPopup(GameState& gs) {
    ...
    std::vector<std::string> text = {
        ...
        "",
        "WASD: Camera | Space: Sidebar | Esc: Menu",
