...
DecisionTreeLayout buildTreeLayout(const Country* player, int screenW, int screenH) {
    ...
    layout.nodeW = std::round(layout.u * 10.0f);
    layout.nodeH = std::round(layout.u * 2.2f);
    layout.scaleFactor = layout.u / 16.0f;
    layout.ySpacing = layout.u * 4.0f * layout.scaleFactor;
    ...
    if (player) {
        for (const auto& node : player->focusTreeEngine.nodes) {
            float wx = node.position.x * layout.scaleFactor;
            float wy = node.position.y * layout.ySpacing;
...
std::pair<float, float> focusToScreen(float fx, float fy,
                                      ...
                                      const Camera& cam) {
    float wx = fx * layout.scaleFactor;
    float wy = fy * layout.ySpacing;
