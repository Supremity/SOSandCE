...
void Popup::draw(SDL_Renderer* r, float uiSize) {
    ...
    for (auto& btn : buttons) {
        ...
        float drawY = btnPressed ? btnTop + 1 : btnTop;

        if (btnTex) {

            SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);
            SDL_SetRenderDrawColor(r, 0, 0, 0, 25);
            SDL_Rect sh = {(int)btnLeft + 1, (int)btnTop + 2, (int)bw, (int)bh};
            SDL_RenderFillRect(r, &sh);

            if (btnPressed) SDL_SetTextureColorMod(btnTex, 150, 150, 155);
            else if (hovered) SDL_SetTextureColorMod(btnTex, 220, 218, 215);
            else SDL_SetTextureColorMod(btnTex, 185, 183, 180);
            UIAssets::draw9Slice(r, btnTex, btnLeft, drawY, bw, bh, 14);
            SDL_SetTextureColorMod(btnTex, 255, 255, 255);

            if (hovered) {
                UIPrim::drawHLine(r, Theme::gold_dim, btnLeft + 8, btnLeft + bw - 8, drawY + 2);
            }
        } else {
            Color btnBg = hovered ? Theme::btn_hover : Theme::btn;
            UIPrim::drawRoundedRect(r, btnBg, btnLeft, drawY, bw, bh, 5,
                                    hovered ? Theme::gold_dim : Theme::border);
        }

        Color tc = hovered ? Theme::gold : Theme::cream;
        UIPrim::drawText(r, btn.label, btnTextSize,
                         bx, (btnPressed ? by + 1 : by), "center", tc, true);
    }
}
