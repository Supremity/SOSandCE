...
bool Engine::init(int width, int height) {
    ...
    if (!fs::exists(assetsPath + "arial.TTF") && !fs::exists(assetsPath + "arial.ttf")) {

        std::string altPath = basePath + "../../assets/";
        if (fs::exists(altPath + "arial.TTF") || fs::exists(altPath + "arial.ttf")) {
    ...
    printf("[Engine] font search: %s\n", (assetsPath + "arial.TTF").c_str());
    fflush(stdout);


    fontPath = assetsPath + "arial.TTF";
    if (!fs::exists(fontPath)) fontPath = assetsPath + "arial.ttf";
    if (!fs::exists(fontPath)) {

        fontPath = basePath + "arial.TTF";
        if (!fs::exists(fontPath)) fontPath = basePath + "arial.ttf";
    }
...
void Engine::recalcUI() {


    uiScale = std::max(12.0f, static_cast<float>(HEIGHT) / 675.0f * 24.0f);
}
